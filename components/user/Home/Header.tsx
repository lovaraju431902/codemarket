...
const menuItems: MenuItem[] = [
  ...
  { name: 'Contact & Help', href: '/contact-help', icon: MdOutlineContactSupport },
  { name: 'Logout', href: '/logout', icon: LogOut },
];

const Header = () => {
  const [isProfile, setIsProfile] = useState<boolean>(true);
  const [open, setOpen] = useState<boolean>(false); // Controlled state for mobile sheet and icon toggle
  const email = "lovarajuk431902@gmail.com"
  const user = "lovarajuk431902"
  const cartsize=0
  const notificationcount=0
        ...
        {isProfile ? (
          <>
            ...
            <div className="block md:hidden">
              <div className="flex items-center gap-4 relative border-none"> {/* Reduced gap-10 to gap-4 for compactness on small screens */}
                <span className="relative">
                  <Badge className={`absolute left-3 bottom-3 w-4 h-4 ${notificationcount>0? "bg-blue-500":"bg-transparent"} `}>{notificationcount>0?notificationcount:''}</Badge>
                  <IoMdNotificationsOutline size={30} className="text-black-600" />
                </span>
                ...
                <Sheet open={open} onOpenChange={setOpen}> {/* Made Sheet controlled */}
                  <SheetTrigger asChild>
                    <Button variant="ghost" className="p-2 rounded-lg"> {/* Removed manual onClick */}
                      {open ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />} {/* Icon now syncs with Sheet state */}
                    </Button>
                  </SheetTrigger>
                  <SheetContent side="left" className="bg-white">
                    <SheetHeader>
                    ...
                    {/* Navigation - Consider adding main nav links here if needed for mobile consistency */}
                    <nav className="flex-1 p-4">
                      <ul className="space-y-1">
                        {menuItems.map((item) => (
                          <li key={item.name}>
